...
int hawkbit_init(void)
{
	...
	flash_dev = device_get_binding(PM_SETTINGS_STORAGE_DEV_NAME);
	if (!device_is_ready(flash_dev)) {
		LOG_ERR("Storage device is not ready");
		return -ENODEV;
	}
...
enum hawkbit_response hawkbit_probe(void)
{
	...
	if (!hb_context.final_data_received) {
		...
		hb_context.code_status = HAWKBIT_DOWNLOAD_ERROR;
	} else if (memcmp(response_hash, hb_context.dl.file_hash, mbedtls_md_get_size(hash_info)) !=
		   0) {
